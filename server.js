...
// Vollständiger Haken-Status für geräteübergreifende Synchronisation
// Struktur: { templates: [...], days: { "YYYY-MM-DD": { checks:{}, extras:[] } }, notes: {} }
let syncState = {
  templates: null,
  days: {},
  notes: {},
...
// ── Endpunkt: vollständigen Sync-State speichern ───
// POST /state
// Body: { templates, days, notes }
app.post('/state', (req, res) => {
  const { templates, days, notes } = req.body;
  if (!templates && !days && !notes) {
    return res.status(400).json({ error: 'Mindestens eines der Felder templates, days oder notes erforderlich.' });
  }
  if (templates !== undefined) syncState.templates = templates;
  if (days !== undefined) {
    // Merge days, nicht überschreiben
    Object.assign(syncState.days, days);
  }
  if (notes !== undefined) syncState.notes = notes;
  syncState.lastUpdate = new Date().toISOString();
  console.log(`[${new Date().toLocaleTimeString('de-DE')}] Sync-State aktualisiert`);
  res.json({ ok: true });
});

// ── Endpunkt: vollständigen Sync-State abrufen ─────
// GET /state
